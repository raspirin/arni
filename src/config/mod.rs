...
pub trait SyncFile {
    ...
    fn sync(&mut self) -> Result<()> {
        ...
        match self.modified_time().duration_since(disk_access_time) {
            ...
            Ok(_) => {info!("Arni believes we don't need to write back.");self.write_back()?},
            ...
            Err(_) => {
                ...
                let on_disk = std::fs::File::open(self.path()).map_err(|e| {warn!("Fail to open on disk file: {}", e); e} )?;
                let on_disk = std::io::read_to_string(on_disk).map_err(|e| {warn!("Fail to read on disk file: {}", e); e})?;
                self.merge(on_disk)?;
                self.write_back()?;
            }
        }
